...
export function CardInformation({nome, numeros, legenda, link}: {nome: string; numeros: number | string; legenda: string; link: string}){
    return(
        <>
            <div className="w-svw bg-[#1b1b1b] bg-gradient-to-l from-transparent to-[#0e0e0e] from-25% rounded-xl relative overflow-hidden p-4
            drop-shadow-md drop-shadow-white/15">
                <h4 className="text-xs md:text-xs font-montserrat font-light text-white">
                    TOTAL COMPLETO DE
                </h4>

                <h1 className="text-lg md:text-2xl font-manrope font-bold text-white">
                    {nome}
                </h1>

                <div className="w-full flex mt-4">
                    <div className="max-w-9/12 w-full flex flex-col gap-2">
                        <h1 className="text-xl md:text-5xl font-montserrat font-semibold text-white">{numeros}</h1>
